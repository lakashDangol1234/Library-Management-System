...
$sid = $_SESSION['sid'];
$sql = "SELECT * FROM `fine` INNER JOIN `students` ON fine.sid = students.sid INNER JOIN `books` ON fine.bid = books.bid WHERE fine.sid = '1' AND fine.status='not paid'";
$stmt = $conn->prepare($sql);
$stmt->execute();
$result = $stmt->fetchAll(PDO::FETCH_OBJ);

if($_SERVER['REQUEST_METHOD']=="POST" && isset($_POST['pay_fine'])){
    $fineId = $_POST['pay_fine'];
    $sql = "UPDATE `fine` SET `status` = 'paid' WHERE `fine_id` = '$fineId'";
    $stmt=$conn->prepare($sql);
    $stmt->execute();
}
?>
        ...
        <div class="container-fluid mt-5">
            <div class="table-responsive border border-2 rounded-3 p-2">
                <table id="book_table" class="table table-bordered display align-middle m-auto bg-white table-hover">
                    <h4 class="border-bottom mt-1 pb-2 mb-3">Fine List</h4>
                    ...
                    <tbody>
                        ...
                        foreach ($result as $row) {
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="ms-lg-3">
                                        <p class="fw-bold mb-1">' . $row->day . '</p>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="ms-lg-3">
                                        <p class="fw-bold mb-1">' . $row->fine_amount . '</p>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <form action="'.$_SERVER['PHP_SELF'].'" method="POST">
                                    <button name="pay_fine" value="'.$row->fine_id.'" class="btn btn-sm btn-primary">
                                        <i class="bi bi-pencil-square"></i>
                                        <span class="d-none d-sm-inline" >Pay fine</span>
                                    </button>
                                </form>
                            </td>
                        </tr>';
