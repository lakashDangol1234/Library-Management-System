...
// $sql = "SELECT * FROM issued_book WHERE sid = $_SESSION['sid'] AND approve='pending'";
$sid = $_SESSION['sid'];
$sql = "SELECT issue_book.issueBook_id,issue_book.bid,approve,books.book_name,books.book_status FROM issue_book INNER JOIN books ON issue_book.bid = books.bid WHERE sid = '$sid' AND approve='pending'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['cancel_request'])) {
    ...
    // Making available if unavailable
    $sql = "SELECT book_status FROM books WHERE `bid` = '$bid'";
    $stmt3=$conn->prepare($sql);
    $stmt3->execute();

    $roww=$stmt3->fetch(PDO::FETCH_OBJ);
    if($roww->book_status == 'unavailable'){
        $sql = "UPDATE `books` SET `book_status` = 'available' WHERE `bid` = '$bid'";
        $stmt4=$conn->prepare($sql);
        $stmt4->execute();
    }

    // Deleting from issue_book table as well
    $sql = "DELETE FROM `issue_book` WHERE `issueBook_id` = '$issueBookId'";
    $stmt5=$conn->prepare($sql);
    $stmt5->execute();

    header("location:".$_SERVER['PHP_SELF']."?requestCancelled=true");
...
<body>
    ...
    <main>
        ...
        <div class="container-fluid mt-5">
            <h3 class="mb-3">Requested Books</h3>
                ...
                <table id="book_table" class="table table-bordered display align-middle m-auto bg-white table-hover">
                    ...
                    <tbody>
                        ...
                        foreach ($result as $row) {
                            ...
                            <input type="hidden" name="issueBook_id" value="' . $row->issueBook_id . '">
                                <button class="btn btn-sm btn-primary" name="cancel_request" value="' . $row->bid . '">
                                    <i class="bi bi-pencil-square"></i>
                                    <span class="d-none d-sm-inline" >Cancel Request</span>
                                </button>
                            </td>
                            </form>
                        </tr>';
